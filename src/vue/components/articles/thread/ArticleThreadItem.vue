...
//     if (!props.item?.fecha) return ''
//     return new Date(props.item.fecha).getFullYear()
// })

const formattedDate = computed(() => {
    if (!props.item?.fecha) return ''
    return new Date(props.item.fecha).toLocaleDateString('es-BO', {
        day: 'numeric',
        month: 'long',
        year: 'numeric'
    })
